...
Adopted 10/26/96, amended 1/10/26

© 1996-2026 International Defensive Pistol Association, Inc. All rights reserved.
...
4. SCORING RULES — 16
5. PENALTY RULES — 23
6. STAGE DESIGN RULES — 25
7. PERMANENTLY PHYSICALLY DISABLED SHOOTER (PPDS) RULES — 28
8. EQUIPMENT RULES — 29
9. CLASSIFICATION RULES ARE LOCATED IN THE MATCH ADMINISTRATION RULES — 36
10. APPEALS PROCESS — 37
...
2.2.3 A firearm with a non-functioning trigger safety or a manual safety.
...
2.2.5.2 While engaging targets in a CoF under the direct supervision and visual contact of an SO.
...
## 2.7 Firearm Carry Condition

2.7.1.1 The normal condition of pistols not in use during a CoF is holstered and unloaded, with hammer down or striker forward and magazine removed or cylinder empty.

2.7.1.2 The normal condition of PCC not in use during a CoF is unloaded with magazine removed. Shooters must use a chamber safety flag when uncased, or clear chamber device, that is externally visible when transporting from to the range or stage to stage.

2.7.1.3 Un-cased pistol caliber carbines must be held in the hands with the firearm vertical. Match Directors may require this to be "vertically upwards" or "vertically downwards" providing this is made clear to all competitors prior to the match. Slings may be used provided the firearm clearly has a chamber flag and the muzzle is oriented in the vertical position at all times when not in use during a stage.

2.7.1.4 Magazines, speed loaders, and moon clips may be reloaded while off the firing line, but the shooter's firearm can be loaded or unloaded only under the direction of the SO.

## 2.8 Start Conditions

Refer to Equipment Appendix.
...
2.9.1.3 Muzzle Exclusion Zone: Muzzle Exclusion Zones must be marked on doors/ports/windows that the shooter is required to open during the CoF with a doorknob/handle. This type of muzzle safe point designates a keep out area and should be a minimum dimension of 6" square and a maximum of 12" square. If the muzzle points at this keep out area while the shooter is touching the marked area doorknob/handle, the shooter will be disqualified.
...
2.11.3.1 A Hot range is defined as a range where each shooter has the choice to carry a loaded firearm at any time. Loaded or unloaded firearms may only be handled while on the firing line and under the direct supervision of an SO.
...
2.12.2.1 When the shooter has proper eye and hearing protection, the SO will issue the Load and Make Ready command. The shooter will prepare the firearm and magazines to match the required start position for the stage. Typically, this is to load the firearm and holster. After loading the firearm, holstering will be performed by the shooter while standing with the proper care to ensure the firearm is free of anything inside the trigger guard and the muzzle is oriented away from the shooter's body for safety. Additional options may include non-typical loading or staging of equipment. The shooter will then assume the starting position necessary for the stage. If the shooter's firearm is not to be loaded for the start of a stage the command used will be "Make Ready." Shooters with optics may confirm it is functional during Load and Make Ready. Any additional actions that appear as a rehearsal are not permitted.
...
2.12.9 If Clear, Slide Forward, Close Cylinder, or Flag it and Bolt Forward

2.12.9.1 Once the Shooter has inspected the chamber/cylinder and found it to be clear, this command will be issued, and the shooter will comply. PCC will insert a chamber flag and release the bolt forward.

2.12.10 Pull the Trigger (Semi Pistol Only)

2.12.10.1 The shooter will point the firearm in a safe direction and pull the trigger to further verify that the chamber is clear. If the firearm fires, the shooter will be disqualified from the match. This requirement also applies to firearms with a de-cocker or magazine disconnect. For firearms with a magazine disconnect, an empty magazine, or dummy magazine must be inserted before the trigger is pulled, and then removed again.

2.12.11 Holster, Muzzle Up, or Muzzle Down

2.12.11.1 The shooter will safely holster a pistol or point the muzzle up or down for PCC.
...
3.2.1 Tactical Priority is a method of target engagement in which targets are engaged by their order of threat.

3.2.1.1 All targets must be engaged in tactical priority, including all targets engaged "in the open."

3.2.1.2 Targets in the open are considered equal threat when the difference in the target distances to the shooter is less than 2 yards.

3.2.1.3 If several targets are visible at the same time, targets are engaged from near-to-far measured from the start position or where the shooter is initially exposed to the targets.
...
3.2.2.1 A cardboard target is deemed to have been engaged when the required number of shots for that target have been fired at the target that is visible to the shooter.
...
3.2.3 Activated Target Priority
...
3.2.3.3 A shooter may not leave a PoC for an activated target until it is activated and engaged.

3.2.3.4 Once a disappearing target is no longer visible (after activation) a shooter may advance without penalty.
...
3.4.2 A "loaded chamber reload" is when the magazine is empty with a loaded chamber.

3.4.3 A "tactical reload" is when the magazine/cylinder is not empty and the shooter initiates a reload while retaining the removed magazine and properly stowing it.

3.4.4 The shooter initiates a reload by performing any one of the following actions:

3.4.4.1 Activating the magazine release on a semi-auto pistol (as evidenced by the magazine falling from the firearm).

3.4.4.2 Opening the cylinder of a revolver.

3.4.5 A firearm is deemed to be reloaded when the magazine is seated and the slide is in battery or the revolver cylinder is closed. The firearm must contain at least one unfired cartridge in the chamber, magazine, or cylinder.

3.4.6 If the shooter "drops" or "racks" the slide prior to leaving a Position of Cover and the slide fails to go fully into battery, this is considered a malfunction and no penalty shall be assessed.

3.4.7 A firearm is deemed empty when there is no live ammunition in the chamber or magazine for semi-autos, or there is no live ammunition in the cylinder for revolvers.

3.4.8 Shooters may not perform a reload which results in a loading device containing ammo being left behind. This is commonly known as a "speed reload", and will result in a Procedural Error penalty being issued.

3.4.8.1 Ejected magazines with ammo do not need to be stowed if spare magazines start staged in a shooting position and the shooter does not leave that position.

3.4.9 Dropping a loaded magazine or speed loader/moon clip does not incur a penalty as long as the shooter retrieves and properly stows the loaded magazine or speed loader/moon clip prior to the firing of the last shot in the string of fire.

3.4.10 When clearing a malfunction, the magazine or speed loader/moon clip and/or ammunition that may have caused the malfunction does not need to be retained by the shooter and will incur no penalty if dropped.

3.4.11 Firearms and magazines must always be loaded to the shooter's division capacity, unless otherwise specified by the CoF.

3.4.12 Firearms and magazines manufactured such that they cannot be loaded to the division capacity may still be used as long as they are loaded to their maximum capacity and meet all other criteria for that division. Refer to 8.1.3 for additional information.
...
3.5.6 When cover is available it must be used while engaging targets, unless the shooter is "in the open" and forced to engage targets "in the open." Shooters may not cross or enter any openings (doorways, open spaces, etc.) to engage targets.

3.5.7 Stages will have one or more of the following cover situations:

3.5.7.1 There is no cover anywhere in the stage, so reloading (emergency, tactical reloads and loaded chamber reloads) are allowed "in the open."
...
3.5.7.4 When moving between two positions of cover, no more than 6 shots may be required on "discovered" or "surprise" targets hidden behind a vision barrier.
...
3.6.4.3 If a fault line is not used at a cover object, that location is not a valid PoC for the CoF.

3.6.4.4 A shooter who engages a target while faulting the line shall be assessed a PE.

3.6.4.5 Other measurement methods for determining cover must not be employed.
...
3.7.2 Unless specified otherwise in the stage description, the default ready position requires the shooter to stand erect with the body relaxed and hands resting naturally at sides for pistol. The default shooting position for PCC is facing downrange at Low Ready (PCC shouldered with the muzzle oriented downward a 45 degrees or available PCC marker). Finger clearly visibly outside of the trigger guard.
...
4.1.1 Unlimited Scoring allows the shooter to shoot at each target as much as deemed necessary, as long as this does not violate other IDPA rules. The best hits on a target are used for score. This gives the shooter the option to make up misses or hits that he or she is not satisfied with to improve their score. When the shooter does not fire enough rounds at a target, the unfired rounds are counted as misses and a Procedural Error penalty is assessed for not following the written stage description
...
4.3.1 If a shooter has started a stage but cannot finish the stage due to a broken firearm, squib, or personal injury the score will be determined by writing down the time and scoring the stage as found by noting all points down (including misses), adding penalties for failing to engage and other applicable penalties. When you receive a beep, you receive a score. If no shots are fired, a 0.01 second raw time will be recorded.

4.3.2 If the SO stopped the shooter for a perceived squib, and it turns out not to be a squib, the shooter will be given a reshoot due to SO interference. If the SO stopped the shooter for a perceived squib, and it is a squib, the score will be determined per as above but no reshoot is given. If a shooter stops himself for a perceived squib and it is not a squib, no reshoot will be given.

## 4.4 Did Not Finish Match (Match DNF)

4.4.1 A shooter that chooses not to shoot a stage will be given a DNF for that stage but may continue to shoot other stages for no total match score. At the completion of the match any shooter with a DNF score on any stage will result in a match DNF for that shooter.
...
4.12.3 Steel "Legs": Stationary steel reactive vertical plates representing target legs that present a target at least 3 inches wide and at least 15 inches tall are allowed. These targets are scored as down zero (-0) if they fall. If the target is left standing it is scored as down five (-5). The calibration zone for this target is the upper 1/2 of the target leg.
...
4.12.12 Moving reactive steel and clay pigeon targets are not allowed.

4.12.13 New targets will be evaluated annually.
...
4.16.2 The Match Director will provide a 9 mm or .38 Special firearm and ammunition that together does not exceed the power factor of any regular division. The same firearm and ammunition combination will be used throughout the match for calibration and calibration challenges with no changes.
...
4.16.9.4 If the target is hit anywhere on the scoreable surface and the target does not react properly, the target calibration will be deemed improper, and the shooter will be given a reshoot after the target is recalibrated.
...
6.2.2.5 If a prop uses something that PCC will not fit, then lay the rifle on top or nearby the prop as safety permits.

6.2.2.6 Specify the shooter start position.

6.2.2.6.1 PCC must not start facing up range.

6.2.2.7 Specify a shooting drill in a standards stage provided the actions needed to perform the drill do not exceed two required shooting actions in a single string of fire. (For example, SHO, WHO, and Freestyle in one string).
...
6.3.6 Positions of Cover at windows less than 24" must be marked with a single fault line in the center of the window.

6.3.7 Match directors will use every effort to insure the effective use of props and stage descriptions to remove opportunities for officiating delays before allowing competitors to shoot the stages for score.
...
6.5.15 Only 1 non-threat target may be used for every 2 threat targets in any string of fire. The number of non-threat targets may be rounded up when not a whole number.
...
6.5.27.5 Non-threat targets in the path of a stationary threat target provide no cover or concealment to a shooter when moving from a position of cover.
...
7.1.1.5 To obtain a medical waiver letter issued by IDPA HQ, contact your Area Coordinator to initiate the process.
...
7.1.2.9 Shooters using wheelchair or crutches may forego usage of a concealment garment.
...
*Specialty Division optional division for Tier 1 - Tier 3 Sanctioned matches.
...
8.1.2.2 In all semi-auto divisions, the shooter will also start with one additional round in the chamber, unless the stage description requires otherwise.

8.1.2.3 The stage description may require reduced magazine loading for standard stages and the initial start of a scenario stage.
...
8.1.3.3 CDP, BUG-S 8 rounds

8.1.3.4 REV, 6 rounds

8.1.3.5 BUG-R 5 rounds

8.1.3.6 PCC 30 rounds*
...
8.1.6.4 Grip safety may be disabled if the firearm has an external safety that prevents sear movement.
...
8.1.7.4 Disconnecting or disabling of safety devices including (but not limited to): manual safeties, firing pin, striker, and hammer blocking safeties except grip safety. 1911 series 80 frames may be used with series 70 slides or vice versa. Revolver actions may not be modified so that the hammer can fall when the cylinder is open.
...
8.2.9.3 All other IDPA equipment rules apply for holsters and loading device holders as well as their placement on the body. Match Directors of Tier 1 matches also have the option to allow shooters with .22 rimfire firearms to begin strings at low ready in lieu of requiring a holster.
...
8.3.4.3 Stock REV, CCP, BUG 105

8.3.4.4 Enhanced REV 155

8.3.4.5 PCC 135

8.3.4.6 Calculate the power factor by multiplying the bullet weight in grains by the bullet velocity in feet per second (fps), divide by 1000, and ignore numbers to the right of the decimal. For example, a 230.1 grain bullet at 794.7 fps: 230.1 x 794.7 / 1000 = 182.86047, or 182 power factor.
...
![Figure 1 - OWB Holster Positions Limits](figures/2026/figure-1.png)

![Figure 2 - IWB Holster Position Limits](figures/2026/figure-2.png)

![Figure 3. Demonstrating measurements per 8.5.1.6.](figures/2026/figure-3.jpg)
...
8.7.8 PCC Gear: Shooters may optionally choose to place additional magazines in pouches to carry with them while shooting stages. If they choose to use magazine pouches without concealment, they may not place more than 2 on their belt which must be worn behind the hip bone. Optionally, shooters may carry magazines in pockets that are below the belt or stowed inside the waistband.
...
8.9.4.1 Flashlights may be used in IDPA competition. Lights may not be attached to the shooter's head, hand, wrist, or arm in any fashion, prior to the start signal. The only exception to this is if the flashlight is provided for the stage.
...
10.3.4.5 If the team does not support or sustain the appeal, the MD decision stands and the AC will forward the appeal forms and fee to IDPA Headquarters.

10.3.4.6 If the team does support the appeal, the decision of the MD is reversed and the AC* will return the appeal fee to the shooter.
